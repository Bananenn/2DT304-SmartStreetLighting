...
const char*  server = "7timer.info";  // Server URL
...
void weatherAPI(void *param) {
  (void) param;
  
    // while (true) {
  //   Serial.println("Hej smuts");
  // }

  // for (;;) {
  //   // Put the code to be executed every 12 seconds here
  //   vTaskDelay(pdMS_TO_TICKS(12000)); // Delay for 12 seconds
  //   Serial.println("Hej smuts");
...
void setup() {
  ...
  // xTaskCreate(weatherAPI, "WEATHERAPI", 2048, nullptr, 1, nullptr);
  ...
  if (!client.connect(server, 443))
    ...
    // Make a HTTP request:
    client.println("GET /bin/civillight.php?lon=14.9&lat=56.9&ac=0&unit=metric&output=json&tzshift=0 HTTP/1.0");
    client.println("Host: 7timer.info");
